...
<v-card class="pending-card">
      ...
      <div class="title-wrap">
        <span class="header-icon"><v-icon size="22">mdi-format-list-checks</v-icon></span>
        <h3>PENDING ACTION</h3>
      </div>
      <div class="header-actions">
        <v-btn class="view-btn" variant="outlined" @click="$router.push('/main/pending-actions')">VIEW ALL</v-btn>
      </div>
    </div>
  ...
  <div v-for="item in items" :key="item.title" class="row">
    ...
    <div class="info">
      ...
      <v-chip size="x-small" :class="item.level">{{item.level}}</v-chip>
    </div>
    <div class="status">
...
<style scoped>
.pending-card{height:560px;padding:18px 18px 8px;border-radius:24px}
...
.title{font-weight:700;font-size:20px;display:flex;align-items:center;gap:12px}
.icon,.item-icon{background:rgb(var(--v-theme-surface-variant));color:rgb(var(--v-theme-primary));border-radius:14px;display:grid;place-items:center}
.icon{width:44px;height:44px;border-radius:50%}
.summary{display:flex;justify-content:space-between;margin:10px 0;color:rgb(var(--v-theme-on-surface-variant))}
.summary b{color:rgb(var(--v-theme-on-background));font-size:18px}
.urgent{color:#ef4444;font-weight:600}
.progress{height:8px;display:flex;overflow:hidden;border-radius:10px}
.progress span{display:block;height:100%}
.urgent-bar{width:40%;background:#ef3f55}.normal-bar{width:40%;background:#f59e0b}.low-bar{width:20%;background:rgba(var(--v-theme-on-surface), .65)}
.legend{display:flex;gap:24px;margin:10px 0;color:rgb(var(--v-theme-on-background))}
.red{color:#ef3f55}.orange{color:#f59e0b}.grey{color:#94a3b8}
.row{height:68px;border-top:1px solid rgba(var(--v-theme-on-surface), .12);display:flex;align-items:center;gap:14px}
.item-icon{width:52px;height:52px}
.info{flex:1;position:relative}
.info strong{display:block;font-size:17px;color:rgb(var(--v-theme-on-surface))}
.info small{color:rgb(var(--v-theme-on-background));display:inline-block;margin-right:10px}
.v-chip{font-size:10px}
.URGENT{color:#ef3f55}.NORMAL{color:#f59e0b}.LOW{color:#94a3b8}
.status{text-align:right;color:#ef3f55}.status small{display:block;color:rgb(var(--v-theme-on-background))}
.arrow{font-size:32px}

@media (max-width:1200px){
 .pending-card{height:auto;min-height:100%;padding:16px;}
}
@media (max-width:768px){
 .summary,.legend{flex-wrap:wrap;}
}

/* DARK MODE OVERRIDE */

:deep(.v-theme--dark) .row {border-color:rgba(255,255,255,.12)!important;}
:deep(.v-theme--dark) .info strong {color:rgb(var(--v-theme-on-surface)) !important;}
:deep(.v-theme--dark) .info small,
:deep(.v-theme--dark) .status small {color:rgba(var(--v-theme-on-surface), .45) !important;}
